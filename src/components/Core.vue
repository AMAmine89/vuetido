        ...
        <v-row class="justify-center">
            <v-list class="tasks-list">
                ...
                v-for="(task, index) in tasksGroupe">
                    <div :class="task.stat === 'Traitment'? '':'text--disabled'"> {{ task.task_name }} </div>
                    <div>
                        <v-chip 
                        :color="chipColor(task.importance)"
                        class="mr-3"> {{ task.importance }} </v-chip>
                        ...
                        :disabled="task.asignTo !== userOnLine">
                            ...
                            v-text="task.stat === 'Traitment'? 'mdi-check' : 'mdi-close'"></v-icon>
                        </v-btn>
                        <v-btn icon @click="modale(task)"
...
export default {
    ...
    computed: {
        tasksGroupe() {
            ...
            if (this.$route.name === "Tasks") return this.tasks.filter(task => task.asignTo  === this.userOnLine)
            if (this.$route.name === "Completed") return this.tasks.filter(task => (task.asignTo  === this.userOnLine) && task.stat  !== 'Traitment' ) 
            return this.tasks
        }
    },
}
