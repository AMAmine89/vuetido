            ...
            <v-tabs grow="grow" class="tabs mt-10">
              ...
              <v-tab-item class="px-5 pb-5">
                <v-form ref="loginForm">
                  ...
                  <v-text-field v-model="password" label="Password" counter
                  :append-icon="showPassword ? 'mdi-eye' : 'mdi-eye-off'"
                  :type="showPassword ? 'text' : 'password'"
                  @click:append="showPassword = !showPassword"></v-text-field>
                  <v-btn color="primary" outlined small :disabled="checkValues('login')" @click="validate('login')" >Login</v-btn>
                </v-form>
              </v-tab-item>
              <v-tab-item  class="px-5 pb-5">
                <v-form ref="registerForm">
                  <v-text-field v-model="email" label="Email" :rules="emailRules"></v-text-field>
                  <v-text-field v-model="username" label="Username" :rules="usernameRules" :counter="10"></v-text-field>
                  <v-text-field v-model="password" label="Password" :rules="passwordRules"
                  :append-icon="showPassword ? 'mdi-eye' : 'mdi-eye-off'"
                  :type="showPassword ? 'text' : 'password'"
                  counter
                  @click:append="showPassword = !showPassword" ></v-text-field>
                  <v-btn color="primary" outlined small :disabled="checkValues('register')" @click="validate('register')">Register</v-btn>
                </v-form>
              </v-tab-item>
            </v-tabs>
          </v-card>
...
export default {
  ...
  data() {
    return {
      ...
      usernameRules: [
        ...
        value => (value && value.length <= 10) || "Username must be less than 10",
        value => /^\S+$/.test(value) || "Spaces are not allowed on Username",
        value => !/[^a-zA-Z0-9]/.test(value) || "Characters are not allowed on Username"
      ],
      passwordRules: [
  ...
  methods: {
    checkValues(loginOrRegister) {
      ...
      if (loginOrRegister === "register") return !(this.email && this.username && this.password)
    ...
    checkInfo(value, loginOrRegister) {
      ...
      if (value) {
        ...
        if (loginOrRegister === 'login') return (this.username === "User" 
                                                && this.password === '123' 
                                                && !localStorage.setItem('username', this.username) && this.$router.push({name: 'Dashboard'})) 
                                                || (this.snackbar = true)
        return !localStorage.setItem('username', this.username) && this.$router.push({name: 'Dashboard'})
      }
    }
  },
  mounted() {
